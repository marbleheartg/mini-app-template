...
import { type HTMLAttributes, forwardRef, type ReactNode, createContext, useContext, useState } from "react"
      ...
      >
        ...
        >
          ...
          <svg
            ...
            strokeWidth="2"
            className={clsx(
              "shrink-0 text-(--text)/50 transition-transform duration-300",
              isOpen && "rotate-180"
            )}
          >
        ...
        >
          <div className="pb-3 px-1 text-(--text) text-xs leading-relaxed">{children}</div>
        </div>
      </div>
    )
  }
